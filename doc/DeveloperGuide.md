...
`* * *` | user | edit a person information | update or rectify wrong entries
`* *` | user | hide [private contact details](#private-contact-detail) by default | minimize chance of someone else seeing them by accident
`* *` | user | add a person with multiple [contact](#contact) | 
`* *` | user | display all persons with given tag | locate a group of person easily
`* *` | user | edit last add command | immediately fix wrong data entry
`* *` | user | undo last command | 
`*` | user with many persons in the address book | sort persons by name | locate a person easily
`*` | user with multiple persons of same full name | (force) add all of them as different person | 
`*` | experienced user | edit specific entry of person information in one line | save time retyping all other unchanged information
